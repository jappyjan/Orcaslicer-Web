...
23. **Boolean `PrintConfig` keys are switches, not flags with a value.**
    ...
    *M6 generalised this: see #26. The `=` form works for every option type and for
...
25. **Vector-valued `PrintConfig` flags do not use the `;` separator the rest of the CLI
...
26. **`--key=value` works for every option type, and is the only form that works for all
    ...
    always.**
27. **`--key=` with an empty value is rejected AND eats the next argument.** MEASURED:
...
28. **`--seam-position rear` is rejected with `Invalid value for option --seam-position`
...
29. **G-code output is byte-for-byte reproducible except for one line.** Two identical
